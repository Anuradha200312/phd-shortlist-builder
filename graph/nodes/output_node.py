    ...
    for i, c in enumerate(candidates, start=1):
        entry = {
            ...
            "confidence_breakdown": c.get("confidence_breakdown") or {
                "orcid_verified": bool(c.get("orcid")),
                "orcid_score": 0.5 if c.get("orcid") else 0.0,
                "faculty_page_confirmed": c.get("faculty_page_confirmed"),
                "faculty_score": 0.5,
                "paper_topic_overlap": c.get("embedding_similarity", 0.0),
                "overlap_score": c.get("embedding_similarity", 0.0),
                "recent_activity": (c.get("last_paper_year") or 0) >= (datetime.now().year - 3),
                "recency_score": 0.5 if (c.get("last_paper_year") or 0) >= (datetime.now().year - 3) else 0.0,
                "eligibility_clear": True,
                "eligibility_score": 0.6,
                "h_index": c.get("h_index", 0),
                "hindex_score": min((c.get("h_index") or 0) / 30.0, 1.0),
                "total_score": c.get("confidence") or c.get("confidence_score", 0.5),
            },
            "match_dimensions": {
